...
router.post('/submit_proofs', async (req: Request, res: Response) => {
  ...
  setTimeout(async () => {
    try {
      ...
      // Get tolerance rules
      const cols = caseResult[0].columns;
      const caseRow = rowToObject(cols, caseResult[0].values[0]);
      const toleranceRules = JSON.parse(caseRow.tolerance_rules_json);
      const expectedQty = caseRow.expected_qty;
      ...
      const onProofsSubmittedMessage: OnProofsSubmittedMessage = {
        ...
        message: {
          verification_case: {
            ...
            order_id: content.order_id,
            state,
            verification_window: JSON.parse(caseRow.window_json),
            required_proofs: JSON.parse(caseRow.required_proofs_json),
      ...
      const onVerificationAcceptedMessage: OnVerificationAcceptedMessage = {
        ...
        message: {
          verification_case: {
            ...
            verification_window: JSON.parse(caseRow.window_json),
            required_proofs: JSON.parse(caseRow.required_proofs_json),
            expected_quantity: { value: caseRow.expected_qty, unit: 'kWh' },
            delivered_quantity: caseRow.delivered_qty ? { value: caseRow.delivered_qty, unit: 'kWh' } : undefined,
            expires_at: caseRow.expires_at,
          },
        },
      };
...
router.post('/settlement_start', async (req: Request, res: Response) => {
  ...
  setTimeout(async () => {
    try {
      ...
      const cols = caseResult[0].columns;
      const caseRow = rowToObject(cols, caseResult[0].values[0]);
      const deliveredQty = caseRow.delivered_qty || order.quote?.totalQuantity || 0;
      const totalQty = order.quote?.totalQuantity || 0;
      const totalPrice = order.quote?.price?.value || 0;
      const pricePerUnit = totalQty > 0 ? totalPrice / totalQty : 0.10; // Default
      const currency = order.quote?.price?.currency || 'USD';
      
      // Calculate settlement amount
      const baseAmount = deliveredQty * pricePerUnit;
      const deviationQty = caseRow.deviation_qty || 0;
      const deviationPercent = caseRow.deviation_percent || 0;
