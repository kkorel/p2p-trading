...
import { Check, Loader2, AlertCircle, CreditCard, AlertTriangle, Package } from 'lucide-react';
import { BottomSheet, Button, Badge } from '@/components/ui';
import { formatCurrency, formatTime } from '@/lib/utils';
import { useBalance } from '@/contexts/balance-context';
import type { BulkSelectedOffer, BulkSelectResponse, Order } from '@/lib/api';

type BulkOrderStep = 'preview' | 'payment' | 'processing' | 'success' | 'error';

interface BulkOrderSheetProps {
  open: boolean;
  onClose: () => void;
  bulkSelection: BulkSelectResponse | null;
...
export function BulkOrderSheet({
  ...
  const [step, setStep] = useState<BulkOrderStep>('preview');
  ...
  const { selectedOffers, summary } = bulkSelection;
      ...
      {step === 'preview' && (
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between py-3 border-t border-[var(--color-border)]">
            <div>
              <p className="text-sm text-[var(--color-text-muted)]">Average Price</p>
              <p className="text-xs text-[var(--color-text-muted)]">
                {formatCurrency(summary.averagePrice)}/kWh
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-[var(--color-text-muted)]">
                  Energy ({summary.totalQuantity} kWh from {summary.offersUsed} offers)
                </span>
      ...
      {step === 'processing' && (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            Reserving {summary.offersUsed} offers...
          </p>
        </div>
      )}
      ...
      {step === 'success' && order && (
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="text-lg font-semibold text-[var(--color-text)] mb-1">
            Bulk Order Successful!
          </p>
          <p className="text-sm text-[var(--color-text-muted)] mb-6">
            {summary.offersUsed} offers confirmed
          </p>
          ...
          <div className="w-full bg-[var(--color-surface)] rounded-[14px] p-4 mb-6">
            ...
            <div className="flex justify-between mb-2">
              <span className="text-sm text-[var(--color-text-muted)]">Total Quantity</span>
              <span className="text-sm font-medium text-[var(--color-text)]">
                {summary.totalQuantity} kWh
              </span>
            </div>
            <div className="flex justify-between mb-2">
              <span className="text-sm text-[var(--color-text-muted)]">Offers</span>
              <span className="text-sm font-medium text-[var(--color-text)]">
                {summary.offersUsed}
              </span>
            </div>
