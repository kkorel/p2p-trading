import { runSettlementBlockOnConfirm, runSettlementVerification } from '../packages/bap/src/callbacks';
...
async function run(): Promise<void> {
  ...
  await runSettlementVerification(order.id, 'SUCCESS', context);
  console.log('PHASE C SMOKE OK');
}

run().catch((error) => {
  console.error('PHASE C SMOKE FAILED', error);
  process.exitCode = 1;
});
